...
def calculate_infusion(patient: Patient, base_solution: Solution, additives: Dict[str, Additive]) -> InfusionMix:
    try:
        ...
        # 必要量の計算
        calculation_steps += f"{step}. **必要量の計算**\n"
        step += 1
        ...
        if fat is not None:
            ...
            calculation_steps += f"    - **脂肪量**\n"
            calculation_steps += f"        - {fat} g/kg/day × {weight} kg = **{total_fat:.2f} g/day**\n"
            input_amounts['脂肪量'] = fat
            input_units['脂肪量'] = "g/kg/day"
        else:
            total_fat = 0.0
            calculation_steps += f"    - **脂肪量**: 計算対象外\n"
        ...
        if na is not None and total_na > 0:
            ...
            if additional_na > 0:
                ...
                additional_na = 0.0
                calculation_steps += f"        - 追加で必要なNa量はありません。\n"
        else:
            base_na_total = 0.0
            additional_na = 0.0
        ...
        if k is not None and total_k > 0:
            ...
            if additional_k > 0:
                ...
                additional_k = 0.0
                calculation_steps += f"        - 追加で必要なK量はありません。\n"
        else:
            base_k_total = 0.0
            additional_k = 0.0
        ...
        if cl is not None and total_cl > 0:
            ...
            if additional_cl > 0:
                ...
                additional_cl = 0.0
                calculation_steps += f"        - 追加で必要なCl量はありません。\n"
        else:
            base_cl_total = 0.0
            additional_cl = 0.0
        ...
        if p_volume > 0:
            if "リン酸Na" in detailed_mix:
                ...
                detailed_mix["リン酸Na"] = p_volume
        if fat_volume > 0:
            detailed_mix[additives["イントラリポス20%"].name] = fat_volume
        if ca_volume > 0:

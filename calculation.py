...
def calculate_infusion(patient: Patient, solution: Solution) -> InfusionMix:
    try:
        ...
        calculation_steps += f"総投与量 (TWI): {patient.twi} mL/kg/day × {weight} kg = {twi:.2f} mL/day\n"
        ...
        calculation_steps += f"\n必要GIR: {gir} mg/kg/min × {weight} kg × 60 min × 24時間 ÷ 1000 = {total_gir:.2f} g/day\n"

        total_amino_acid = amino_acid * weight  # g/day
        calculation_steps += f"必要アミノ酸量: {amino_acid} g/kg/day × {weight} kg = {total_amino_acid:.2f} g/day\n"

        total_na = na * weight  # mEq/day
        calculation_steps += f"必要Na量: {na} mEq/kg/day × {weight} kg = {total_na:.2f} mEq/day\n"

        total_k = k * weight  # mEq/day
        calculation_steps += f"必要K量: {k} mEq/kg/day × {weight} kg = {total_k:.2f} mEq/day\n"

        total_p = p * weight  # mmol/day
        calculation_steps += f"必要P量: {p} mmol/kg/day × {weight} kg = {total_p:.2f} mmol/day\n"
        ...
        calculation_steps += f"\nブドウ糖濃度: {solution.glucose_percentage}% (={glucose_per_ml} g/mL)\n"
        ...
        calculation_steps += f"\nアミノ酸濃度: {amino_acid_concentration} g/mL\n"
        ...
        calculation_steps += f"\nベース製剤からのNa量: {solution.na} mEq/L × {glucose_volume:.2f} mL/day ÷ 1000 = {base_na_total:.2f} mEq/day\n"
        ...
        calculation_steps += f"\nリン酸NaからのNa濃度: {na_per_ml_phospho} mEq/mL\n"
        ...
        calculation_steps += f"\nベース製剤からのK量: {solution.k} mEq/L × {glucose_volume:.2f} mL/day ÷ 1000 = {base_k_total:.2f} mEq/day\n"
        ...
        calculation_steps += f"\nKClからのK濃度: {kcl_k_concentration} mEq/mL\n"
        ...
        calculation_steps += f"\nリン酸NaからのP濃度: {p_per_ml_phospho} mmol/mL\n"
        ...
        calculation_steps += f"\n計算された総液量: {glucose_volume:.2f} + {amino_acid_volume:.2f} + {phospho_na_volume:.2f} + {kcl_volume:.2f} + {p_volume:.2f} = {calculated_total_volume:.2f} mL\n"
        ...
        if water_volume < 0:
            calculation_steps += f"\n警告: 総液量がTWIを超過しています。製剤の配合を見直してください。\n"
